...
const AdminSidebarLink = ({ to, icon, label }: any) => {
    ...
    return (
        <Link to={to} className={`flex items-center gap-4 p-4 rounded-xl text-xs font-black uppercase tracking-[0.1em] transition-all ${active ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/30' : 'text-slate-500 hover:text-white hover:bg-white/5'
            }`}>
                ...
                <div className="p-4 lg:p-8 bg-slate-900/60 border-t border-white/10">
                    <div className="flex items-center gap-6 mb-5">
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] transition-all border ${isSystemMsg ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-white/5 border-white/5 text-slate-500 hover:text-white'
                                }`}
                        >
                            SYSTEM PROTOCOL
                        </button>
...
const AdminClients = ({ projects }: { projects: Project[] }) => {
    ...
    const fetchClients = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/auth/clients`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
    ...
    const handleDeactivateClient = async (clientId: string) => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/auth/clients/${clientId}/deactivate`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                        ...
                        <div className="flex flex-wrap gap-2 pt-4">
                            {STAGES.map(s => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest border transition-all ${p.stage === s ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-950 border-white/5 text-slate-600 hover:text-white'
                                        }`}
                                >
                                    {s}
                                </button>
